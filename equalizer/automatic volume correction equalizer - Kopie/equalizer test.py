...
adjust_x = np.convolve(np.random.randn(fs*adjust_duration), h_inv)

mydadjusted = np.convolve(adjust_x,h)
myd= np.convolve(np.random.randn(fs*adjust_duration), h)

#plt.plot(abs(np.fft.fft(np.random.randn(fs))))
#plt.plot(abs(np.fft.fft(mydadjusted*np.hamming(len(mydadjusted)))))
#plt.figure(100)
#plt.plot(abs(np.fft.fft(myd*np.hamming(len(myd)))))

#find the right adjustment factor
while max_rec < 0.3 or max_rec >0.4:
...
# Signals
x = np.random.randn(fs*duration)#*np.hamming(fs*duration) # playback signal, duration seconds of white Gaussian noise
x_equalized = np.convolve(x,h_inv)*noisefactor
#    print(adjusted_signal)
#    idx_max = np.argmax(h_inv)
#    x_equalized = x_equalized[idx_max:len(x_equalized) + 1 - len(h_inv) + idx_max]
#d = np.convolve(x,h)
...
d = sd.playrec(x_equalized, samplerate = fs, channels = 1,device = [1,7])
time.sleep(duration+2)
######d = np.convolve(x_equalized,h)
######d += np.random.randn(len(d))*0.01  
#
#plt.figure(1000)
#plt.plot(d)
#plt.figure(10001)
#plt.plot(x_equalized)
#plt.figure(1002)
#plt.plot(h_inv)
...
for kSample in range(len(d)-10000):
...
file = open("residual error2.npy", "wb")
