...
H_inv[int(len(H_inv)*(0/fs)):int(len(H_inv)*((2000)/fs))] = np.ones(int(len(H_inv)*((2000)/fs))-int(len(H_inv)*(0/fs)))*mean_2_bis_4_kHz
...
file = open("h of system2.npy", "wb")
np.save(file,h_hat,allow_pickle = False,fix_imports = False)
file.close()


file = open("equalizer schreckstimulus.npy", "wb")
