...
def check_output_signal(signal):
    ...
    if len(values_above_one) > 0.004*len(signal):
        ...
        raise ValueError("The Signal that is supposed to be played has too many values above 1. this will corrupt the output!")
...
class Measurement(QtCore.QObject):
    ...
    def play_gpias_gap(self, noiseFreqMin, noiseFreqMax, noiseTime, noise_type = 1):
        ...
        #delay trigger because otherwise trigger is 14.8125 ms to early
        #14.8125 is correct for samplingrates of 48000 and 96000 at the stxII
        preStimArray, burstArray, triggerArray = self.adjust_trigger_timing(preStimArray, burstArray, triggerArray, ms=14.8125)
    ...
    def play_asr_prepuls(self, preStimFreq, preStimAtten, ISI):
        ...
        preStimArray[get_num_samples(ISI-timeToNoiseBurst):get_num_samples(ISI-timeToNoiseBurst) + len(prepuls)]= prepuls
    ...
    def perform_measurement(self, duration_ms):
        ...
        analog_input.CreateAIVoltageChan(b'Dev2/ai0:5',b'',daq.DAQmx_Val_Cfg_Default,-10.,10.,daq.DAQmx_Val_Volts,None)
...
#
#        # DAQmx Read Code
        analog_input.ReadAnalogF64(num_data_points,duration_ms/1000,daq.DAQmx_Val_GroupByChannel,self.data,6*num_data_points,ctypes.byref(read),None)
#
    ...
    def extract_data(self,data, data_extracted, rate, idxStartle):      
        ...
        for trigger in data_trigger:
            ...
            if trigger > thresh: # here is the trigger and the sound begins
                ...
                data_extracted[idxStartle][0]=data_x[int(i-0.8*rate):int(i-0.8*rate)+len(data_extracted[idxStartle][0])]-offset_x_mean
                data_extracted[idxStartle][1]=data_y[int(i-0.8*rate):int(i-0.8*rate)+len(data_extracted[idxStartle][1])]-offset_y_mean
                data_extracted[idxStartle][2]=data_z[int(i-0.8*rate):int(i-0.8*rate)+len(data_extracted[idxStartle][2])]-offset_z_mean

                data_extracted[idxStartle][3]=data_trigger[int(i-0.8*rate):int(i-0.8*rate)+len(data_extracted[idxStartle][3])]
                data_extracted[idxStartle][4]=data_prestim_audio[int(i-0.8*rate):int(i-0.8*rate)+len(data_extracted[idxStartle][4])]
                data_extracted[idxStartle][5]=data_burst_audio[int(i-0.8*rate):int(i-0.8*rate)+len(data_extracted[idxStartle][5])]
