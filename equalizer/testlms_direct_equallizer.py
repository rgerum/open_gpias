...
print(" ")
# Signals
s = np.random.randn(fs*duration)# playback signal, 10 seconds of white Gaussian noise
y = s
x = np.convolve(h,s) # recorded microphone signal; here computed by convolution of the playback signal x with the impulse response h (see above)
...
for kSample in range(len(s)):
    
    buffer_in = np.concatenate((x[kSample:kSample+1], buffer_in[:-1]))
    a = buffer_in.transpose()
    y_hat = np.dot(buffer_in.transpose() , h_hat) # filter input with h_hat, 
    e[kSample] = y[kSample] - y_hat # compute error signal by difference of estimated and true signal
...
h_eq = np.convolve(h,h_hat)# equalized impulse response

#sound = np.zeros(44000*5)
#for i in range(len(sound)):
#    sound[i] = np.sin(i*np.pi*2*440/44000)
import sounddevice as sd
sound = sd.rec(5*40000,samplerate = 40000,channels = 1)
sd.play(sound,40000)
import time 
time.sleep(6)
sound2 = np.convolve(np.convolve(sound.transpose()[0],h), h_hat)
