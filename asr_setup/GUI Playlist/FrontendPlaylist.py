# -*- coding: utf-8 -*-
"""
Created on Thu Nov  9 10:12:35 2017

@author: rahlfshh
"""
import sys
from PyQt5.QtWidgets import (QWidget, QGridLayout, QFileDialog, QPushButton,
     QApplication, QLabel, QLineEdit,QButtonGroup, QRadioButton, QToolTip,
     QMessageBox,QDialog, QDialogButtonBox)
from PyQt5.QtGui import QIntValidator, QFont
import copy
import numpy as np
import BackendPlaylist


##DiaqlogClass to recive Schwellwert Messung Data
#This class is abled to get all needed parameter for a hearing thresold measurement
...
class HearingThresholdDialog(QDialog):
    
    #initializing the Dialog
    def __init__(self, parent = None):
        super(HearingThresholdDialog, self).__init__(parent)
        self.setWhatsThis("Please Enter all Paramters for the hearing threshold. All Fields have a tool tip. If you place the mouse arrow above them it will tell you about the Values you are allowed to enter.")
        ...
        self.grid.addWidget(QLabel(label),0,0,1,10)
        
        #Line Edit which sets the smallest frequenz for which a 
        #hearing threshold is tested
        self.smallestFrequency = QLineEdit(self)
        self.smallestFrequency.setValidator(QIntValidator(350,20000))
        tip = "Enter a value inbetween 350 and 20000. This is the lowest frequency for which a hearingthreshold is tested."
        self.smallestFrequency.setToolTip(tip)
        self.grid.addWidget(QLabel("lowest frequency"),2,0)
        self.grid.addWidget(self.smallestFrequency,2,1)
        self.grid.addWidget(QLabel("[Hz]"),2,2)
        
        #Line Edit which sets the highest frequenz for which a 
        #hearing threshold is tested
        #as the frequencies are measured in oktave/halfe oktave or quater
        #oktave steps it often happens, that the highest frequency isn't tested
        self.highestFrequency = QLineEdit(self)
        self.highestFrequency.setValidator(QIntValidator(350,20000))
        tip = "Enter a value inbetween 350 and 20000. This is the highest frequency for which a hearingthreshold is tested."
        self.highestFrequency.setToolTip(tip)
        self.grid.addWidget(QLabel("highest Frequency"),3,0)
        self.grid.addWidget(self.highestFrequency,3,1)
        self.grid.addWidget(QLabel("[Hz]"),3,2)
        
        #define the steps in which the frequency is increased
        #one oktave means doubling the frequency
        self.oktav = QRadioButton("Octave steps")
        self.halbOktav = QRadioButton("Half octave steps")
        self.viertelOktav = QRadioButton("Quarter octave steps")
        #adding buttons to a button group to connect them logically
        group_buttons = QButtonGroup()
        group_buttons.addButton(self.oktav,1)
        group_buttons.addButton(self.halbOktav,2)
        group_buttons.addButton(self.viertelOktav,3)
        ...
        #hearing threshold is tested
        #could be changed to be konstant in future
        self.maximumPressureLevel = QLineEdit(self)
        self.maximumPressureLevel.setValidator(QIntValidator(10,70))
        tip = "Enter a value inbetween 10 and 70. This is the biggest noiselevel of the prestimulus. The lowest noiselevel is always no noise at all."
        self.maximumPressureLevel.setToolTip(tip)
        self.grid.addWidget(QLabel("maximum noiselevel prestim"),7,0)
        self.grid.addWidget(self.maximumPressureLevel,7,1)
        self.grid.addWidget(QLabel("[dB SPL]"),7,2)
        
        #attenuation level inbetween different hearing threshold measurements
        self.stepSizePressureLevel = QLineEdit(self)
        self.stepSizePressureLevel.setValidator(QIntValidator(1,70))
        tip = "Enter a value inbetween 1 and 70. This is the resolution of the different noiselevels."
        self.stepSizePressureLevel.setToolTip(tip)
        self.grid.addWidget(QLabel("noiselevel steps prestim"),8,0)
        self.grid.addWidget(self.stepSizePressureLevel,8,1)
        self.grid.addWidget(QLabel("[dB SPL]"),8,2)

        
        self.n = QLineEdit(self)
        self.n.setValidator(QIntValidator(1,50))
        tip = "Enter a value inbetween 1 and 50. This is the number of repeats for every measurement."
        self.n.setToolTip(tip)
        self.grid.addWidget(QLabel("repetitions:"),9,0)
        self.grid.addWidget(self.n,9,1)
        self.grid.addWidget(QLabel("[]"),9,2)
    ...
    #check wether all the parameters are entered correctly and handle it 
    #appropriately
    #the allowed values are declared by the intvalidators defined in __init__
    def checkAccept(self):
        ...
        if not self.n.hasAcceptableInput():
            ...
            self.warning("maximum noiselevel prestim is not within the allowed range")
        elif not self.stepSizePressureLevel.hasAcceptableInput():
                self.warning("noiselevel steps prestim is not within the allowed range")
        elif  (parameter[1]>parameter[2]):
            self.warning("lowest frequency is bigger than highest frequency")
        #if no warning occured this dialog is accepted otherwhise it wont close
        #but give a warning
        else:
            self.accept()
    ...
    def parameter(self):
        #frequenzFactor hold a value in [-1,0,1,2]
        #-1 means no radiobutton is checked
        #these buttons are used to define how big the frequency steps are
        frequenzFactor = -1
        if self.oktav.isChecked():
            frequenzFactor=0
        if self.halbOktav.isChecked():
            frequenzFactor=1
        if self.viertelOktav.isChecked():
            frequenzFactor = 2
    ...
    def adjust(self):
        result = self.exec_()
        return (self, self.parameter(), result == QDialog.Accepted)
    ...
    def getParameter(parent = None):
        ...
        return (dialog, dialog.parameter(), result == QDialog.Accepted)
...
class TurnerDialog(QDialog):
    def __init__(self, parent = None):
        super(TurnerDialog, self).__init__(parent)
        #self.setWhatsThis("hallo")
        self.setWhatsThis("Please enter the paramters for the GPIAS-Measurements. Press add band middel, if you want to use a different frequency. press delete if you want to delete one.\nEnter a - befor the bandmiddel to stimulate with notched noise e.g.:-1000\nenter 0 as bandmiddel to use broad band noise") 
        ...
        #one oktave means doubling the frequency
        self.oktav = QRadioButton("+- one octave around band middel")
        self.halbOktav = QRadioButton("+- a half octave around band middel")
        self.viertelOktav = QRadioButton("+- a quarter octave around band middel")
        self.achtelOktav = QRadioButton("+- an eighth octave around band middel")
        #adding buttons to a button group to connect them logically
        ...
        self.group_buttons = QButtonGroup()
        ...
        self.n = QLineEdit()
        self.n.setValidator(QIntValidator(1,50))
        self.nLabel = QLabel("repetitions:")
        
        #Button to add an noiseband
        self.addButton = QPushButton("Add noise band")
    ...
    def addCycle(self):
        ...
        bandMitte.setValidator(QIntValidator(-20000,20000))
        ...
        self.setCycles.append([QLabel("band middel:"),
                               bandMitte,
                               QLabel("[Hz]"),
                               deleteButton])
        #redrawUI
...
class PlaylistGenerator(QWidget):
    ...
    def initUI(self):
        ...
        self.SchwellMessungButtonAdd = QPushButton("Add measurements which determine hearing thresholds")
        ...
        self.TurnerButtonAdd = QPushButton("Add GPIAS Measurements")
        ...
        self.generateButton = QPushButton("Save playlist")
        ...
        def selectFile():
            ...
            if (self.SchwellParameterList == [] and 
                ...
                path = QFileDialog.getSaveFileName(filter = "byteType (*.npy)",
        ...
        self.timeNeededApproxamitely = QLineEdit("The measurements with the current setting take roundabout 0 minutes")
        self.timeNeededApproxamitely.setReadOnly(True)
        self.grid.addWidget(self.timeNeededApproxamitely,3,0)
        
        self.move(300, 150)
        self.setWindowTitle('Startle Playlist Konfiguration')
        self.show()
    ...
    def addHearingThresholdDialog(self):
        ...
        if ok:
            ...
            self.SchwellMessungAdjust = QPushButton("Change measurements which determine hearing threshold")
            self.SchwellMessungAdjust.clicked.connect(
                    self.adjustHearingThresholdDialog)
            
            self.SchwellMessungRemove = QPushButton("Delete measurements which determine hearing threshold")
    ...
    def addTurnerDialog(self):
        ...
        if ok:
            ...
            self.TurnerAdjust = QPushButton("Change GPIAS Measurements")
            self.TurnerAdjust.clicked.connect(self.adjustTurnerDialog)
            
            self.TurnerRemove = QPushButton("Delete GPIAS Measurements")
    ...
    def update_time_needed(self):
        ...
        if self.SchwellParameterList!= []:
            ...
            timeSchwelle = 0
        self.timeNeededApproxamitely.setText("The measurements with the current setting take roundabout " + str(int((timeSchwelle + timeTurner)/60)+1) + " minutes")
