# -*- coding: utf-8 -*-
"""
Created on Fri Jan 12 12:40:04 2018
...
file = open("equalizer kleiner lautsprecher.npy", "rb")
h_inv = np.load(file,allow_pickle = False,fix_imports = False)
file.close()

# Signals
x = np.random.randn(fs*duration)/20#*np.hamming(fs*duration) # playback signal, duration seconds of white Gaussian noise
print(max(x))
#d = np.convolve(x,h)
...
d = sd.playrec(np.convolve(x,h_inv), samplerate = fs, channels = 1,device = [1,7])
